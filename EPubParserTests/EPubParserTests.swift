...
class EPubParserTests: XCTestCase {
    ...
    func testUnzip() {
        ...
        do {
            _ = try EPubHelper.entriesOfZip(atPath: path1, password: nil)
        ...
        do {
            ...
            let entries = try EPubHelper.entriesOfZip(atPath: path2, password: nil)
            XCTAssertTrue(!entries.isEmpty && entries.count == results.count)
            for i in 0..<results.count {
                XCTAssertTrue(entries[i] == results[i])
            }
            
            let data = try EPubHelper.dataOf(zipPath: path2, atEntryPath: "mimetype", password: nil)
            XCTAssertTrue(String(data: data, encoding: String.Encoding.ascii)! == "application/epub+zip")
            
            _ = try EPubHelper.dataOf(zipPath: path2, atEntryPath: "OEBPS", password: nil)
        } catch UnzipError.fileNotFound(let path) {
        ...
        do {
            _ = try EPubHelper.entriesOfZip(at: url1, password: nil)
        ...
        do {
            _ = try EPubHelper.entriesOfZip(at: url2, password: nil)
        ...
        do {
            let entries = try EPubHelper.unzip(atPath: path2, toPath: toPath, password: nil)
